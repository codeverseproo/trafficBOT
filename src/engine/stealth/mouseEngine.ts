...
import type { JitterProfile, BehaviorProfile } from '../behaviorProfiles';
...
export class MouseEngine {
  ...
  private static jitterConfig: Record<JitterProfile, { tremor: number; hesitation: number; overshoot: number }> = {
  ...
  private static gaussian(mean: number, sd: number): number {
    let u = 0, v = 0;
  ...
  private static buildPath(start: Point, end: Point): Point[] {
  ...
  private static buildSaccadePath(start: Point, end: Point): Point[] {
  ...
  // ─── Main move ────────────────────────────────────────────────────────────
  static async move(page: Page, x0: number, y0: number, x1: number, y1: number, profile?: BehaviorProfile): Promise<Point> {
  ...
  static async hoverRandom(page: Page, pos: Point): Promise<Point> {
  ...
  static async moveToElement(page: Page, selector: string, pos: Point, profile?: BehaviorProfile): Promise<Point> {
  ...
  static async humanClickElement(page: Page, selector: string, pos: Point, profile?: BehaviorProfile): Promise<Point> {
    return this.moveToElement(page, selector, pos, profile);
  }

  // ─── Attention Heatmap — drift toward content anchors ────────────────────
  static async attentionDrift(page: Page, pos: Point, profile?: BehaviorProfile): Promise<Point> {
    // Find headings/images that attract attention
    const anchors = await page.evaluate(() => {
      return Array.from(document.querySelectorAll<HTMLElement>('h2, h3, h4, strong, img, figure'))
        .map(el => {
    ...
    if (anchors.length === 0) return pos;
