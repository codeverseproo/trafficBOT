...
exports.ScrollingEngine = void 0;
class ScrollingEngine {
    static easeOutQuint(t) { return 1 - Math.pow(1 - t, 5); }
    // ─── Inertial humanScroll (mouse wheel) ──────────────────────────────────
    static async humanScroll(page, distance, _profile) {
    ...
    static async readingDwell(page, profile, fatigue) {
        const wordCount = await page.evaluate(() => {
            const texts = Array.from(document.querySelectorAll('p, li, blockquote'))
                .filter(el => {
        ...
        const dwell = Math.max(profile.dwellMin, Math.min(profile.dwellMax * fatigueMult, baseMs * fatigueMult));
        await new Promise(r => setTimeout(r, dwell));
    }
    // ─── Image / media focus pause ────────────────────────────────────────────
    static async imagesPause(page, profile) {
        const imgCount = await page.evaluate(() => {
            return Array.from(document.querySelectorAll('img, figure, video'))
                .filter(el => {
                const r = el.getBoundingClientRect();
    ...
    // ─── Full page read ───────────────────────────────────────────────────────
    static async readPage(page, profile, fatigue, maxTimeMs = 25000, adPositions = []) {
        const prof = profile ?? {
            wpm: 238, dwellMin: 1200, dwellMax: 4000,
            reverseScrollChance: 0.2, earlyStopChance: 0.05,
            imagePauseMultiplier: 1.0,
        };
        const fat = fatigue ?? { dwellMultiplier: () => 1, speedMultiplier: () => 1 };
        ...
        while (scrolled + vpHeight < bodyHeight) {
            ...
            if (Math.random() < prof.earlyStopChance)
                break;
        }
