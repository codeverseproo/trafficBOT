...
class AdEngine {
    ...
    static async detectGoogleAds(page) {
        const result = await page.evaluate(() => {
            ...
            const isFilledAdIframe = (f) => {
                const id = f.id || '';
                const src = f.src || '';
                const h = f.offsetHeight;
                const w = f.offsetWidth;
                if (h < 10 || w < 10)
                    return false; // collapsed = unfilled
                // Case 1: GPT frame identified by id (src stays blank when filled via doc.write)
                if (id.startsWith('aswift_') || id.startsWith('google_ads_iframe'))
                    return true;
            ...
            document.querySelectorAll('ins.adsbygoogle').forEach(el => {
                ...
                const hasFilled = status === 'filled' ||
                    Array.from(el.querySelectorAll('iframe')).some(isFilledAdIframe);
                if (hasFilled) {
            ...
            document.querySelectorAll('[id*="google_ads"], [id*="gpt-ad"], [id*="div-gpt-ad"]')
                .forEach(el => {
                if (el.offsetHeight < 10 || el.offsetWidth < 10)
                    return;
                const hasFilledIns = el.querySelector('ins.adsbygoogle[data-ad-status="filled"]') !== null;
                const hasFilledIframe = Array.from(el.querySelectorAll('iframe')).some(isFilledAdIframe);
            ...
            // Sticky ad detection
            const hasStickyAds = Array.from(document.querySelectorAll('iframe')).some(f => {
    ...
    static async triggerLazyAds(page, workerId) {
    ...
    static async handleGDPRConsent(page, workerId) {
        const selectors = [
    ...
    static async dwellOnStickyAds(page, seconds = 5) {
        const hasSticky = await page.evaluate(() => Array.from(document.querySelectorAll('iframe')).some(f => {
            const s = window.getComputedStyle(f);
    ...
    static async checkAdBlockerDetected(page) {
    ...
    static async scrollToAdsAndDwell(page, adPositions) {
    ...
    static async handleVignetteAd(page, workerId) {
        try {
            const vignetteFound = await page.evaluate(() => {
                ...
                return iframes.some(f => {
                    ...
                    const isAd = f.id.startsWith('aswift_') ||
                        ...
                        (f.src && (f.src.includes('googlesyndication') ||
            ...
            const stillVisible = await page.evaluate(() => Array.from(document.querySelectorAll('iframe')).some(f => {
                const style = window.getComputedStyle(f);
    ...
    static async findInternalContentLink(page) {
        return page.evaluate(() => {
            ...
            const candidates = Array.from(document.querySelectorAll('a'))
                ...
                try {
                    ...
                    if (u.pathname === currentPath)
                        return false; // don't re-visit same page
    ...
    static async waitForLoad(page, workerId) {
        const tag = `[W${workerId}] [AdEngine]`;
        // ── Helper: wait for page to be fully loaded ──────────────────────────
        const waitFullLoad = async (label) => {
        ...
        const triggerAndPoll = async (label) => {
